...
import com.msw.mesapp.activity.fragment.warehouse.FragmentSampleOut1;
import com.msw.mesapp.activity.fragment.warehouse.FragmentSampleOut2;
...
//样品入库
public class SampleOutActivity extends AppCompatActivity {
    ...
    private final String[] mTitles = {"未领取", "已领取"};
    ...
    public void initSlidingTabLayout() {
        fragmentList.add(new FragmentSampleOut1()); //样品未领取
        fragmentList.add(new FragmentSampleOut2()); //样品已领取
        viewPager.setNestedpParent((ViewGroup) viewPager.getParent());//将 viewpager 的父view传递到viewpager里面 ,解决滑动冲突
        viewPager.setAdapter(new SampleOutActivity.mPageAdapter(this.getSupportFragmentManager()));
