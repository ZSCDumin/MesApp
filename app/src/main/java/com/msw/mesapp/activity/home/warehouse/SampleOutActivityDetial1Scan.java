...
public class SampleOutActivityDetial1Scan extends AppCompatActivity {
    ...
    private void initData() {
        ...
        ToastUtil.showToast(SampleOutActivityDetial1Scan.this, headcode, ToastUtil.Success);
                ...
                for(int i =0;i<batchList.size();i++){
                    ...
                    for(int j =0;j<list.size();j++){
                        ...
                        if((map1.get("2").toString()).equals(map2.get("2").toString()) ) {
                            map.put("3", "1"); batchList.set(i,map); break;
                        }
                        else
                        if(j == list.size()-1 ) {
                            map.put("3", "0"); batchList.set(i,map); BtFlag=false;
                        }
                        else
                            continue;
                    }
                    ...
                    EasyHttp.post(GlobalApi.WareHourse.MaterialIn.PATH_Send_Updata)
                            ...
                            .execute(new SimpleCallBack<String>() {
                                ...
                                public void onSuccess(String result) {
                                    ...
                                    if (code == 0) {
                                        ToastUtil.showToast(SampleOutActivityDetial1Scan.this, message, ToastUtil.Success);
                                        finish();
                                    } else {
                                        ToastUtil.showToast(SampleOutActivityDetial1Scan.this, message, ToastUtil.Error);
                                    }
                                }
                                @Override
                                public void onError(ApiException e) {
                                    ToastUtil.showToast(SampleOutActivityDetial1Scan.this, GlobalApi.ProgressDialog.INTERR, ToastUtil.Confusion);
                                }
                            });
            }
        });
